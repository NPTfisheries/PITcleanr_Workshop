...
PIT-tag data is often difficult to quickly summarize into meaningful information due to the large amount tag observations. For instance, PTAGIS serves as a data repository containing a single record for every observation of a tag code; including the initial detection, or "mark", when the tag is implanted, and all subsequent detections on each antenna the tag passes, or at recapture (e.g., at weirs), and recovery (e.g., carcass surveys) sites. The number of detections that exist for each individual tag code can easily be in excess of 1,000 observations. Therefore, querying PTAGIS for all detections of lots of tags often lends to a wealth of data that is unwieldy to process. `PITcleanr` aims to "compress" or minimize the data to a more manageable size, without losing any of the information contained in the full dataset.

This document serves as a tutorial to the most common functions contained with `PITcleanr` and provides some simple data summaries that may help you get started using the package, and performing your own PIT-tag data analyses. 

## Installation

`PITcleanr` can be installed from GitHub using the R `remotes` package. Additional information on package installation can be found on the [website README](https://github.com/KevinSee/PITcleanr?tab=readme-ov-file#installation-instructions). 
...
# Query PTAGIS Meta Data

## Interrogation Sites

You can use `PITcleanr` to query and download metadata for PTAGIS interrogation sites using the function `queryInterrogationMeta()`. The `site_code` argument is used to specify sites; alternatively, set `site_code` to `NULL` to retrieve metadata for all sites.
...
`PITcleanr` includes the function `queryTestTagSite()` which can be used to query and download the test tag history for one INT site and an entire calendar year. The test tag information can be useful for an initial evaluation of site operations.
...
The `queryCapHist()` function queries the complete tag history for one tag from PTAGIS via DART's mirrored data repository.
...
#DART no longer provides mark event information with a tag code's observation data. , only observations after the mark event are returned.
...
If desired, the `queryMRRDataFile()` function can be used to query a raw MRR file from PTAGIS. NOTE: only the latest corrected version of a MRR file is returned.
...
Using the `purrr::map()` function, the user could iterate over `queryMRRDataFile()` to easily query multiple MRR tag files and combine them into a single data frame or list.
...
The mark data from `queryMRRDataFile()` and the observations data from `queryCapHist()` could be combined to form a complete tag history. However, this process not recommended for large number of tag codes. The suggested approach is to read in a complete tag history file that has previously been queried and downloaded from PTAGIS.


# Read Complete Tag Histories
...
<!-- There is an errant site plotted when running plotNodes() on the corrected parent-child, which seems to be causing some issues below, as well. -->
...
```{r build-paths}
# build paths to nodes
node_paths = buildPaths(parent_child = parent_child_nodes)
...
knitr::purl(input = "C:/Git/PITcleanr_Workshop/docs/practical_examples.Rmd",
            output = "C:/Git/PITcleanr_Workshop/docs/practical_examples.R")
```